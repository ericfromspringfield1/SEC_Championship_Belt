...
export default function TeamPage({ params }: { params: { team: string } }) {
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-2xl font-bold"><TeamLabel team={team} className="text-2xl font-bold" logoClassName="h-9 w-9" /></h1>
          ...
          <div className="space-y-2">
            {summary.wins.map((win: any) => (
              <div key={win.id} className="rounded border p-2 text-sm">
                ...
                <p>Defeated <TeamLabel team={win.loserTeam} className="text-sm" logoClassName="h-4 w-4" /> ({win.winnerScore}-{win.loserScore})</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-2">
            {summary.losses.map((loss: any) => (
              <div key={loss.id} className="rounded border p-2 text-sm">
                ...
                <p>Lost to <TeamLabel team={loss.winnerTeam} className="text-sm" logoClassName="h-4 w-4" /> ({loss.loserScore}-{loss.winnerScore})</p>
              </div>
            ))}
          </div>
        </div>
