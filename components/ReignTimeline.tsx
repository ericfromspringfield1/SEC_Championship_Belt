    ...
    <div className="space-y-2">
      {reigns.map((reign) => (
        <div key={`${reign.champ}-${reign.start_date}`}>
          <div className="text-xs"><TeamLabel team={reign.champ} className="text-xs" logoClassName="h-4 w-4" />: {reign.start_date} → {reign.end_date ?? 'Present'} ({reign.length_days} days)</div>
